...
function Get-ADUsers {
    ...
    try {
        ...
        foreach ($r in $found) {
            ...
            $statusColor = if ($disabled) { 'DarkGray' } elseif ($locked) { 'Red' } else { 'Green' }
            $flags = @()
            ...
            Write-Host "  [USER] $($obj.SamAccountName)$flagStr" -ForegroundColor $statusColor
        }
        Write-Host "[+] Found $($results.Count) user(s)" -ForegroundColor Green
    }
    catch {
        Write-Host "[-] User enumeration error: $_" -ForegroundColor Red
    }
    finally {
        $found.Dispose()
        $searcher.Dispose()
