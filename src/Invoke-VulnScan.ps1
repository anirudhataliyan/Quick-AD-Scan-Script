...
function Invoke-VulnScan {
    ...
    Write-Host "[*] Running LDAP vulnerability checks on $Server ..."

    $findings = @()

    #  Check 1: Anonymous bind 
    Write-Host "  [CHECK 1] Anonymous LDAP bind..."
    $found = $null
    try {
        ...
        Write-Host "  [!] VULNERABLE: Anonymous LDAP bind succeeded - disable null sessions!"
        $findings += [PSCustomObject]@{ Check='AnonymousBind'; Result='VULNERABLE'; Detail='Anonymous bind accepted' }
    }
    catch {
        Write-Host "  [+] Anonymous bind rejected (OK)"
    ...
    Write-Host "  [CHECK 2] LDAP signing requirement..."
    $found = $null
    try {
        ...
        Write-Host "  [!] POTENTIALLY VULNERABLE: Server accepted unsigned bind - LDAP signing may not be enforced."
    ...
    try {
        ...
        if ($success) {
            Write-Host "  [+] LDAPS port 636 is open."
            $findings += [PSCustomObject]@{ Check='LDAPS'; Result='OK'; Detail='Port 636 open' }
        }
        else {
            Write-Host "  [!] LDAPS port 636 is NOT reachable - only plain LDAP available."
            $findings += [PSCustomObject]@{ Check='LDAPS'; Result='WARNING'; Detail='Port 636 unreachable' }
        }
    }
    catch {
        Write-Host "  [!] LDAPS check failed: $_"
        $findings += [PSCustomObject]@{ Check='LDAPS'; Result='ERROR'; Detail=$_.Exception.Message }
    }

    #  Check 4: LDAP channel binding hint 
    Write-Host "  [CHECK 4] Checking for LDAP channel binding (remote hint)..."
    # We can't read the remote DC's registry, so we flag this as a manual check
    Write-Host "  [i] Channel binding (CVE-2017-8563) cannot be tested remotely without a Kerberos token."
    Write-Host "      Verify LdapEnforceChannelBinding = 2 on the DC via Group Policy."
