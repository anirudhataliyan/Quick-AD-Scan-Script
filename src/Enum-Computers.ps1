...
function Get-ADComputers {
    ...
    try {
        ...
        foreach ($r in $found) {
            ...
            $color = if ($disabled) { 'DarkGray' } elseif ($isStale) { 'Yellow' } else { 'Green' }
            $flags = @()
            if ($disabled) { $flags += 'DISABLED' }
            if ($isStale)  { $flags += 'STALE(90d)' }
            $flagStr = if ($flags) { " [$($flags -join '|')]" } else { '' }

            Write-Host "  [PC] $($obj.Name)  OS: $($obj.OS)$flagStr" -ForegroundColor $color
        }
        Write-Host "[+] Found $($results.Count) computer(s)" -ForegroundColor Green
    }
    catch {
        Write-Host "[-] Computer enumeration error: $_" -ForegroundColor Red
    }
    finally {
        $found.Dispose()
        $searcher.Dispose()
