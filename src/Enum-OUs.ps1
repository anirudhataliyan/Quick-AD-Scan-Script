...
function Get-ADOUs {
    ...
    Write-Host "[*] Enumerating Organisational Units..." -ForegroundColor Cyan
    ...
    try {
        ...
        foreach ($r in $found) {
            ...
            Write-Host "  $indent[OU] $($obj.Name)$gpoStr" -ForegroundColor $(if ($hasGPO) { 'Yellow' } else { 'White' })
        }
        Write-Host "[+] Found $($results.Count) OU(s)" -ForegroundColor Green
    }
    catch {
        Write-Host "[-] OU enumeration error: $_" -ForegroundColor Red
    }
    finally {
        $found.Dispose()
        $searcher.Dispose()
