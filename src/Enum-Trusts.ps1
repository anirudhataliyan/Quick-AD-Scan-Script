...
function Get-ADTrusts {
    ...
    try {
        ...
        if ($found.Count -eq 0) {
            Write-Host "  [i] No domain trusts found." -ForegroundColor DarkGray
        }
        ...
        foreach ($r in $found) {
            ...
            $color = if ($obj.Direction -eq 'Bidirectional') { 'Red' } else { 'Yellow' }
            Write-Host "  [TRUST] $($obj.TrustedDomain)  Dir: $($obj.Direction)  Type: $($obj.Type)" -ForegroundColor $color
        }
        Write-Host "[+] Found $($results.Count) trust(s)" -ForegroundColor Green
    }
    catch {
        Write-Host "[-] Trust enumeration error: $_" -ForegroundColor Red
    }
    finally {
        $found.Dispose()
        $searcher.Dispose()
